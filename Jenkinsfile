node() {
   stage('Build'){
	...
	cmd_exec("docker-compose -p appmourad -f _init\\docker\\docker-compose_dev.yml build --no-cache")
	...
	bat "docker exec appmourad bash -c 'bash \\var\\www\\tmp\\scripts\\install.sh'"
   }
}
